...
namespace SimpleEntityFramework.Test
{
    ...
    ///     </author>
    /// </summary>
    public static class SqlServerHepler
    {
        #region 执行SQL，返回被操作的行数,public static int ExecuteNonQuery(string _connectionString, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        ...
        #region 执行SQL，返回被操作的行数,public static int ExecuteNonQuery(SqlTransaction trans, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        ...
        #region 执行SQL，返回SqlDataReader，public static SqlDataReader ExecuteReader(string _connectionString, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        ...
        {
            return ExecuteReader(connection, CommandType.Text, cmdText);
        }

        #endregion

        #region 执行SQL，返回DataTable， public static DataTable ExecuteDataTable(string _connectionString, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        ...
        /// <param name="commandParameters">参数数组</param>
        /// <returns></returns>
        public static DataTable ExecuteDataTable(SqlConnection connection, CommandType cmdType, string cmdText,
            params SqlParameter[] commandParameters)
        {
            var dt = new DataTable();
            var cmd = new SqlCommand();
        ...
        #region 返回第一行第一列，public static object ExecuteScalar(string _connectionString, CommandType cmdType, string cmdText, params SqlParameter[] commandParameters)
        ...
        public static bool ExecuteTransaction(SqlConnection conn, List<string> cmdTextes,
            List<SqlParameter[]> commandParameterses)
        {
            var flag = false;
            if (cmdTextes.Count == commandParameterses.Count)
            {
                ...
                try
                {
                    for (var i = 0; i < cmdTextes.Count; i++)
                    {
        ...
        #region 以事务执行sql语句列表，返回事务执行是否成功，public static bool ExecuteTransaction(string _connectionString, List<string> cmdTextes, List<SqlParameter[]> commandParameterses)
        ...
        /// <param name="commandParameterses">sql语句列表对应的参数列表，参数列表必须与sql语句列表匹配</param>
        /// <returns></returns>
        public static bool ExecuteTransaction(string connectionString, List<string> cmdTextes,
            List<SqlParameter[]> commandParameterses)
        {
            var flag = false;
            if (cmdTextes.Count == commandParameterses.Count)
